...
public protocol VideoProcessingChainDelegate: AnyObject {
...
public struct VideoProcessingChain {
    ...
    /// upstream publisher.
    /// - Tag: humanBodyHandGestureRequest
    private let humanBodyHandGestureRequest = VNDetectHumanHandPoseRequest()

    /// The action classifier that recognizes exercise activities.
    private let actionClassifier = GestureRecognizer.shared
...
extension VideoProcessingChain {
    ...
    /// - Returns: A `HandGesture` array if `VNDetectHumanBodyHandGestureRequest` succeeds
    /// and its `results` property isn't `nil`; otherwise `nil`.
    ///
    /// The method also sends the frame and any poses in it to the delegate.
    /// - Tag: findHandGesturesInFrame
    private func findHandGesturesInFrame(_ frame: CGImage) -> [HandGesture]? {
        ...
        do { try visionRequestHandler.perform([humanBodyHandGestureRequest]) } catch {
            assertionFailure("Human HandGesture Request failed: \(error)")
        }

        let poses = HandGesture.fromObservations(humanBodyHandGestureRequest.results)

        return poses
    }

    /// Returns the largest pose by area.
    /// - Parameter poses: A `HandGesture` array optional.
    /// - Returns: The largest`HandGesture` when the array isn't empty; otherwise `nil`.
    /// - Tag: isolateLargestHandGesture
    private func isolateLargestHandGesture(_ poses: [HandGesture]?) -> HandGesture? {
    ...
    /// - Tag: multiArrayFromHandGesture
    private func multiArrayFromHandGesture(_ item: HandGesture?) -> MLMultiArray? {
    ...
    private func predictActionWithWindow(_ currentWindow: [MLMultiArray?]) -> ActionPrediction {
        ...
        let filledWindow: [MLMultiArray] = currentWindow.map { multiArray in
            if let multiArray = multiArray {
                ...
                return HandGesture.emptyHandGestureMultiArray
            }
        }
        ...
        guard poseCount >= minimum else {
            return ActionPrediction.noHandPredication
        }
